...
class EnhancedBallTrajectoryAnalyzer:
    ...
    #     using physics-based interpolation for smooth transitions.
    #     """
    #     if not self.trajectory_fit:
    #         raise ValueError("Must run analysis first")
    #
    ...
    #         # This provides smooth derivatives of position data
    #         from scipy.signal import savgol_filter
    #
    ...
    #         pred_velocities = np.zeros_like(full_trajectory)
    #         pred_accelerations = np.zeros_like(full_trajectory)
    #
    #         for i in range(2):  # For both x and y components
    #             pred_velocities[1:-1, i] = (full_trajectory[2:, i] - full_trajectory[:-2, i]) / (2 * dt)
    #             pred_velocities[0, i] = pred_velocities[1, i]
    #             pred_velocities[-1, i] = pred_velocities[-2, i]
    #
    #             pred_accelerations[1:-1, i] = (pred_velocities[2:, i] - pred_velocities[:-2, i]) / (2 * dt)
    #             pred_accelerations[0, i] = pred_accelerations[1, i]
    #             pred_accelerations[-1, i] = pred_accelerations[-2, i]
    #
    ...
    #         ball_scatter = ax2.scatter([], [], c='blue', s=200)
    #
    #         # Plot observed points
    #         ax2.scatter(smooth_observed[:, 0], smooth_observed[:, 1], color='red', alpha=0.3, s=30, label='Observed')
    #
    ...
    #                 end_vel = pred_velocities[current_frame]
    #                 start_acc = obs_accelerations[current_frame]
    #                 end_acc = pred_accelerations[current_frame]
    #
    ...
    #
    #             return [im1, ball_scatter, metrics_box]
    #
    #         # Create animation
    ...
    # def shooting_method_interception(self, shooter_pos, target_trajectory, dt=0.01):
    #     """
    #     Improved implementation of numerical shooting method for interception.
    #     """
    #
    #     def forward_shooting(params):
    #         """Solve forward IVP with given parameters."""
    #         v0, theta, t_shoot = params
    #
    #         # Initial conditions
    #         x0, y0 = shooter_pos
    #         vx0 = v0 * np.cos(theta)
    ...
    #         state = np.array([x0, y0, vx0, vy0])
    #
    #         # Integrate until interception time
    #         t = 0
    #         trajectory = [state[:2]]
    #
    #         while t < t_shoot:
    #             state = self.rk4_step(t, state, dt, k_m_ratio=0.02)
    #             trajectory.append(state[:2])
    #             t += dt
    #
    #             if state[1] < 0:  # Ground hit check
    #                 return np.array(trajectory)
    #
    #         return np.array(trajectory)
    #
    #     def boundary_conditions(params):
    #         """Evaluate boundary conditions with better error handling."""
    #         trajectory = forward_shooting(params)
    #         if len(trajectory) < 2:
    #             return np.array([1e6, 1e6])  # Large but finite error
    #
    #         t_shoot = params[2]
    #         target_idx = min(int(t_shoot / dt), len(target_trajectory) - 1)
    #         target_pos = target_trajectory[target_idx]
    #         final_pos = trajectory[-1]
    #
    #         # Add penalty for ground hits
    #         ground_penalty = 1e3 if final_pos[1] < 0 else 0
    #         return final_pos - target_pos + np.array([0, ground_penalty])
    #
    #     def jacobian(params):
    #         """Compute Jacobian with improved numerical stability."""
    #         eps = 1e-4  # Increased perturbation size
    #         J = np.zeros((2, 3))
    #         F0 = boundary_conditions(params)
    #
    #         for j in range(3):
    #             params_perturbed = params.copy()
    #             # Scale perturbation to parameter magnitude
    #             eps_scaled = eps * max(1.0, abs(params[j]))
    #             params_perturbed[j] += eps_scaled
    #             F_perturbed = boundary_conditions(params_perturbed)
    #             J[:, j] = (F_perturbed - F0) / eps_scaled
    #
    #         return J
    #
    ...
    #
    #     # Direct path guess
    #     t_guess = np.sqrt(2 * np.hypot(dx, dy) / g)
    #     theta_guess = np.arctan2(dy, dx)
    #     v0_guess = np.hypot(dx / t_guess, dy / t_guess + 0.5 * g * t_guess)
    #     initial_guesses.append([v0_guess, theta_guess, t_guess])
    #
    #     # High arc guess
    #     theta_high = np.pi / 3  # 60 degrees
    #     v0_high = np.sqrt((dx ** 2 * g) / (2 * np.cos(theta_high) ** 2 *
    #                                        (dx * np.tan(theta_high) - dy)))
    #     t_high = dx / (v0_high * np.cos(theta_high))
    #     initial_guesses.append([v0_high, theta_high, t_high])
    #
    ...
    #         try:
    #             # Newton's method with improved convergence checks
    #             max_iter = 50
    #             tolerance = 0.1
    #
    #             for iteration in range(max_iter):
    #                 F = boundary_conditions(params)
    ...
    #                 delta = np.linalg.lstsq(J.T @ J + 1e-2 * np.eye(3), -J.T @ F, rcond=None)[0]
    #
    #                 # Improved line search
    #                 alpha = 1.0
    #                 for _ in range(10):  # Maximum line search iterations
    #                     new_params = params + alpha * delta
    #
    #                     # Improved constraints
    #                     new_params[0] = np.clip(new_params[0], 1.0, 100.0)  # velocity
    #                     new_params[1] = np.clip(new_params[1], -np.pi / 2, np.pi / 2)  # angle
    #                     new_params[2] = np.clip(new_params[2], 0.1, 10.0)  # time
    #
    #                     new_error = np.linalg.norm(boundary_conditions(new_params))
    #                     if new_error < error * (1 + 1e-4):  # Allow slight increase
    #                         params = new_params
    #                         break
    #                     alpha *= 0.5
    #
    #                 if alpha < 1e-4:
    #                     break  # Exit if line search fails
    #
    #         except Exception as e:
    #             print(f"Initial guess {guess} failed: {str(e)}")
    #             continue
    #
    #     if best_solution is None:
    #         raise ValueError("No valid solution found with any initial guess")
    #
    #     # Get final trajectory
    #     v0, theta, t_shoot = best_solution
    #     trajectory = forward_shooting(best_solution)
    #
    #     return theta, v0, trajectory, t_shoot


if __name__ == "__main__":
    analyzer = EnhancedBallTrajectoryAnalyzer()
    video_path = "media/cut_test2.mp4"
    ...
    if success:
        ...
        try:
            ...
            if not success:
                ...
                animation = analyzer.create_animation(
                    video_path=video_path,
                    output_path="animation_with_shooting.gif",
                    fps=30,
                    shooting_data={
...
# if __name__ == "__main__":
#     analyzer = EnhancedBallTrajectoryAnalyzer()
#     video_path = "media/cut_test2.mp4"  # Your test video path
#     output_path = "output_trajectory.avi"
#
#     # Run analysis
#     success = analyzer.analyze_video(video_path, output_path)
...
#             ]
#
#             # Try different shooter positions
#             success = False
#             best_error = float('inf')
#             best_solution = None
#
...
#                 print(f"\nTrying shooter position: {pos}")
#                 try:
#                     # Call the shooting method with the proper signature
#                     angle, v0, shooting_trajectory, shooting_delay = analyzer.shooting_method_interception(
#                         shooter_pos=pos,
#                         target_trajectory=full_trajectory,
#                         dt=analyzer.dt  # Use the same dt as the analyzer
#                     )
#
#                     # Calculate error metric for this solution
#                     min_distance = float('inf')
