...
class EnhancedBallTrajectoryAnalyzer:
    ...
    def plot_trajectory_comparison(self, ax):
        """
        Add observed trajectory points to the plot and create a comparison visualization
        with proper axis limits to show all data points
        """
        try:
            # Convert observed positions to meters
            observed = np.array(self.positions) * self.pixel_to_meter
            ...
            # Flip y coordinates to match physics convention
            observed[:, 1] = frame_height * self.pixel_to_meter - observed[:, 1]

            # Plot observed points
            ax.scatter(observed[:, 0], observed[:, 1],
                       color='red', alpha=0.8, s=30,
                       label='Observed')

            # Generate predicted trajectory for full time range
            t_flight = max(self.timestamps[-1] * 2.0, 2.0)  # Extend prediction time
            t_points = np.linspace(min(self.timestamps), t_flight, 100)
            predicted = self.predict_full_trajectory(t_points)
            ...
            # Calculate bounds that include both observed and predicted points
            all_x = np.concatenate([observed[:, 0], predicted[:, 0]])
            all_y = np.concatenate([observed[:, 1], predicted[:, 1]])

            x_min, x_max = np.min(all_x), np.max(all_x)
            y_min, y_max = np.min(all_y), np.max(all_y)

            # Add margins (20% of range)
            ...
            self.logger.error(f"Error in plot_trajectory_comparison: {e}")

    def create_animation(self, video_path: str, output_path: str = None, fps: int = 30):
        """
        Create enhanced animation with seamless trajectory integration while maintaining original layout
        """
        if not self.trajectory_fit:
            raise ValueError("Must run analysis first")
        ...
        try:
            ...
            # Calculate frame duration in milliseconds for animation
            interval = 1000 / (original_fps / 2)  # Slow down to half speed
            ...
            t_flight = max(self.timestamps[-1] * 1.5, 2.0)  # Extend beyond last observation
            ...
            self.plot_trajectory_comparison(ax2)
            ax2.grid(True)
            ...
            ball_radius = 10
            ball_scatter = ax2.scatter([], [], c='blue', s=(ball_radius * 2) ** 2)

            # Plot observed points
            ax2.scatter(observed[:, 0], observed[:, 1], color='red', alpha=0.3, s=30, label='Observed')

            # Plot predicted trajectory
            valid_trajectory = ~np.isnan(full_trajectory).any(axis=1)
            valid_points = full_trajectory[valid_trajectory]

            if len(valid_points) > 0:
                # Draw full trajectory path
                ax2.plot(valid_points[:, 0], valid_points[:, 1], 'g--', alpha=0.5, label='Predicted')
            ...
            def update(frame):
                ...
                # Update ball position using integrated trajectory
                if frame < len(full_trajectory) and valid_trajectory[frame]:
                    # Use observed positions during observation period
                    if frame < len(observed):
                        current_pos = observed[frame]
                    else:
                        # Smoothly transition to predicted trajectory
                        current_pos = full_trajectory[frame]

                    ball_scatter.set_offsets(np.c_[current_pos[0], current_pos[1]])
            ...
            anim = animation.FuncAnimation(
                fig,
                update,
                frames=min(len(frames), len(full_trajectory)),
                interval=interval,
                blit=True
            )
