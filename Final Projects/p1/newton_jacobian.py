...
class BallShooting:
    def __init__(self):
        ...
        self.k_m_ratio = 0.1  # air resistance coefficient / mass ratio
    ...
    def integrate_trajectory(self, initial_state, target_x, target_y, target_radius):
        ...
        try:
            ...
            for i in range(max_steps):
                ...
                # Debug prints
                if i % 1000 == 0:
                    print(f"Step {i}: Position = ({state[0]:.2f}, {state[1]:.2f})")
                    print(f"Target = ({target_x:.2f}, {target_y:.2f})")
                    print(
                        f"Distance to target = {np.sqrt((state[0] - target_x) ** 2 + (state[1] - target_y) ** 2):.2f}")

                # check for target hit
                if self.check_ball_hit(state[0], state[1], target_x, target_y, target_radius):
                    print(f"Target hit at step {i}")
                    return trajectory[:i + 1]

                # check bounds with debug info
                try:
                    if self.is_out_of_bounds(state, target_x, target_y):
                        print(f"Trajectory out of bounds at step {i}")
            ...
            print("Maximum steps reached without hitting target")
            return trajectory[:i + 1]
...
if __name__ == "__main__":
    ...
    try:
        ...
        fig, anim = game.simulate_game('media/balls.jpg')
