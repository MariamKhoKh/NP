...
# Capture the video
cap = cv2.VideoCapture('Media/market-square.avi')
if not cap.isOpened():
    raise IOError("Error: Unable to open the video file.")

# Define consistent frame dimensions
consistent_width, consistent_height = 640, 480
...
for i in frame_indices:
    ...
    ret, frame = cap.read()
    if ret:
        frame = cv2.resize(frame, (consistent_width, consistent_height))  # Resize frames
        frames.append(frame)
...
frame_width, frame_height = consistent_width, consistent_height
...
while True:
    ...
    frame = cv2.resize(frame, (consistent_width, consistent_height))
    ...
    for contour in contours:
        if cv2.contourArea(contour) > 200:  # Filter small contours
            ...
            for object_id, data in tracked_objects.items():
                ...
                if distance < min_distance and distance < 80:  # Threshold for matching
...
fps = cap.get(cv2.CAP_PROP_FPS)  # Use the original video's FPS
